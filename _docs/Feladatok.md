...
- [ ] 62. **StateNode badge hover-expand**: a badge alapállapotban a 2-betűs kódot mutatja (kompakt node), hoverre maga a badge kinyúlik és a teljes nevet mutatja (pl. „FA" → „Fájl elérhető"). Nincs popup/tooltip, nincs delay — inline morph. Implementáció: két `<span>` (rövid kód + teljes név), CSS-ben `:hover` átvált láthatóságot + szélesség átmenettel, VAGY egyetlen `<span>` `data-label` attribútummal és `::after`-rel. A natív `title` attribútum megszűnik (aria-label marad accessibility-hez). [StateNode.jsx](../packages/maestro-dashboard/src/features/workflowDesigner/nodes/StateNode.jsx) `validatorBadge`/`commandBadge` + [workflowDesigner.css](../packages/maestro-dashboard/src/features/workflowDesigner/workflowDesigner.css) `.state-node__badge`.
