...
- [ ] **A.2.1** A.1.9 hard contract szerver-oldali enforcement-je: `update_workflow_compiled` CF (vagy a workflow-mentő endpoint) `unknown_group_slug` 400 error-t ad, ha a workflow összes slug-mezőjének uniója nem subset-je a `requiredGroupSlugs[].slug`-nak. A kliens-oldali compiler ugyanezt menti előtt validálja.
- [ ] **A.2.2** `activate_publication` CF kibővítés: (a) autoseed minden hiányzó `requiredGroupSlugs` slug-ra `groups` doc létrehozása (üresen — `slug` + `label` + `description` + `color` + `isContributorGroup` + `isLeaderGroup` a workflow-ból); (b) minden seedelt/létező csoportban legalább 1 `groupMembership` ellenőrzés → ha nincs, **409 `empty_required_groups`** + lista.
- [ ] **A.2.3** `assign_workflow_to_publication` CF (vagy `update_publication`): a hozzárendelés pillanatában is autoseed (de nem követeli meg a min. 1 tagot — az csak aktiválásnál).
- [ ] **A.2.4** Snapshot: `compiledWorkflowSnapshot.requiredGroupSlugs` rögzítődik az aktiválás pillanatában (a futó pub immune marad a workflow változásra).
- [ ] **A.2.5** `remove_group_member` CF: ha az utolsó tag eltávolítása + a csoport slug-ja egy aktív pub workflow-jának `requiredGroupSlugs`-ban szerepel → warning + notification (engedi, snapshot védi a runtime-ot).
- [ ] **A.2.6** `update_group_metadata` CF: `slug` immutable — csak `label`, `description`, `color`, `isContributorGroup`, `isLeaderGroup` szerkeszthető.
- [ ] **A.2.7** `archive_group` / `delete_group` CF: blokk, ha bármely aktív pub vagy nem-archivált workflow `requiredGroupSlugs`-ban szerepel a slug → 409 `group_in_use` + lista. **Plusz**: blokk, ha bármely `articles.contributors` vagy `publications.defaultContributors` JSON-mezőben **kulcsként** szerepel a slug (slug→userId mapping) — minta: `removeUserFromGroup` CF már a member-eltávolítás után takarítja az orphan kulcsokat (`packages/maestro-server/functions/invite-to-organization/src/main.js:2261-2425`); itt fordítva, hivatkozás-detektálás és blokk.
- [ ] **A.2.8** `bootstrap_organization` refactor: a 7-csoport default seedelés kivétele. (Permission set default seed marad — A.3.2.)
