...
- [/] **S.13.3** — Error message info-disclosure audit. **Phase 1.0 partial close 2026-05-15** (Codex pre + 2 stop-time MAJOR-loop + verifying): centralized `fail(res, code, reason, extra)` helper [helpers/util.js](../packages/maestro-server/functions/invite-to-organization/src/helpers/util.js) deep-strip-eli a sensitive top+nested `error`/`message`/`details`/`stack`/`cause` mezőket az `extra`-ban + `redactValue` deep-pass PII-pattern catch a többi mezőre. `main.js:726` top-level catch raw `err.message` → `'internal_error'` reason + `executionId`. 2 ad-hoc `success: true` fix: `invites.js:837` (batch results[].reason) + `schemas.js:3414` (backfill_organization_status stats.errors[].error). **Phase 1.5 (külön iter)**: maradék 12+ `success: true` body leak (`schemas.js` stats.errors[].message minta, `orgs.js:850` orgCleanup.error, `_finalizeOrgIfProvisioning` raw e.message). **Phase 2 (külön iter)**: maradék 10-15 CF. Részletek: [[Komponensek/LoggingMonitoring]].
