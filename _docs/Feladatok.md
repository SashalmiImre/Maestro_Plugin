...
- [x] **S.7.7d** — `.github/workflows/sdk-compatibility-check.yml` CI workflow. Trigger: `package.json` / `yarn.lock` / per-CF `package.json` változás PR-en. Jobs: (1) CF main.js + helpers `node --check` syntax-check globstar-rel + S.13.3/S.7.7b build-generator drift-check (`yarn check:cf-validator` + `cf-orphan-guard` + `cf-response-helpers`); (2) `yarn audit` info-only continue-on-error. `set -e` fail-fast a syntax-job-on. ✅ 2026-05-16 Closed.
...
- [/] **S.13.4** — Monitoring alertek design 2026-05-16: 5 alert-rule (A1 CF-failure, A2 login-fail, A3 rate-limit, A4 WS disconnect, A5 invite-rate anomaly) + Slack/PagerDuty/email channels + runbook + **platform-agnostic JSON-DSL pseudocode** per alert (2026-05-16 PM bővítés) — adaptálható Better Stack alert-rule YAML / Grafana Loki LogQL / custom Node.js webhook-listener-re. Új jegyzet: [[Komponensek/MonitoringAlerts]] ("Implementation pseudocode" szekció). Implementation Phase 3 (USER-TASK 13 trigger).
