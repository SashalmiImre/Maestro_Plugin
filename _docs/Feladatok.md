...
- [x] 103. **`fetchWorkflow` scope-race guard szimmetria** (2026-04-22, #98 follow-up, halasztva komment + gen-ref):
    ...
    - **Döntés**: a list-override race halasztva (pre-existing, kis valószínűségű, nincs user-riport). A halasztást rögzítő WHY-komment bekerült a `fetchWorkflow` fölé ([DataContext.jsx:316-324](packages/maestro-dashboard/src/contexts/DataContext.jsx:316)) + a `fetchWorkflowGenRef` deklaráció kommentje ([DataContext.jsx:80-85](packages/maestro-dashboard/src/contexts/DataContext.jsx:80)). Ha valaha elővesszük: a mostani `fetchWorkflowGenRef`-et `setWorkflows`/`seedWorkflowVersions` ELŐTTI post-await guard-dal kell bővíteni — a #98 scope-key + gen-kombót NE másoljuk (redundáns, a gen-counter önmagában elég).
    - **Mellék-fix**: az L-104 `workflowsLoading` flag bevezetésével megszületett egy új loading-race (régebbi fetch finally-je kioltja a frissebb fetch loading-ját). A `fetchWorkflowGenRef` ezt a finally-ben gen-guard-dal lezárja ([DataContext.jsx:351-357](packages/maestro-dashboard/src/contexts/DataContext.jsx:351)). A harden 2. iterációban a Codex review + adversarial egyaránt flaggelte ezt; a javítás minimális (gen-ref + finally if-guard), szimmetrikus az archivált fetch `archivedFetchGenRef.current === gen` mintájával.
    - **Harden**: Iter 0 simplify CLEAN, Iter 1 Codex P2 + adversarial MEDIUM új loading-race → javítva. Iter 2 Codex review CLEAN, Claude CLEAN, Codex adversarial flaggelte a pre-existing list-override race-t (L-103 halasztva, dokumentált out-of-scope).

- [x] 104. **Workflow lista loading state (aktív + archivált)** (2026-04-22, #98 follow-up, minor UX):
    - A #98 megoldás eagerly fetcheli az archivált workflow-kat scope-váltáskor. Fetch közben az `archivedWorkflows` üres tömb, a Panel tab-content a `Nincs archivált workflow.` empty state-et mutatta a `Betöltés…` helyett.
    - **Implementálva**: két új state a `DataContext`-be — `workflowsLoading` + `archivedWorkflowsLoading` (initial `true`), `fetchWorkflow` / `fetchArchivedWorkflows` start-nél `true`, finally-ben `false`. A scope-null guard-ág szinkron `false`-ra állít. A `fetchArchivedWorkflows` finally-je a meglévő `archivedFetchGenRef.current === gen` mintát követi (A→B→A race-védelem), a `fetchWorkflow` a most bevezetett `fetchWorkflowGenRef`-et használja (L-103 mellék-fix).
    - **UI**: [WorkflowLibraryPanel.jsx:487-499](packages/maestro-dashboard/src/components/workflows/WorkflowLibraryPanel.jsx:487) az empty-state div-ben új felső ág: `(tab === 'archived' ? archivedWorkflowsLoading : workflowsLoading) ? 'Betöltés…' : …` — a szűrő/empty-state szövegek az eddigi sorrendben maradnak alatta. Az aktív listára is kiterjesztve a szimmetria kedvéért.
    - **Nem érintett**: a start-kori `setArchivedWorkflows([])` clear megmarad (a komment azt indokolja: hibaágon ne maradjon az előző scope listája). A loading flag a sikeres útvonalon elfedi a pillanatnyi üres-állapotot, így nincs felesleges villanás.
    - **Harden**: Iter 1 codex P2 + adversarial MEDIUM: a `workflowsLoading` finally-je stale fetch-ben kioltja a frissebb loading-ot → `fetchWorkflowGenRef` + finally gen-guard. Iter 2: codex review CLEAN, Claude CLEAN, adversarial HIGH flaggelte a pre-existing L-103 list-override race-t (halasztva, out-of-scope).
