...
- [ ] **S.7.7b** — `documentSecurity: true` flag verify a 6 érintett collection-en (Codex stop-time BLOCKER 1, ADR 0014 Layer 1 prerequisite). Lista: `articles`, `publications`, `layouts`, `deadlines`, `userValidations`, `systemValidations`. Appwrite Console manuális verify VAGY új helper CF action (pl. `verify_collection_document_security`). Ha bármelyik flag NEM `true`, a doc-szintű ACL ignorálódik → collection-szintű `read(users)` továbbra is rétegesen exponál. **DEPLOY-BLOCKER** S.7.7 close előtt.
- [ ] **S.7.7c** — Legacy backfill a 6 user-data collection-re (Codex stop-time BLOCKER 2). A S.7.7 fix csak a friss `createRow`/`createDocument` hívásokra hat — a S.7.7 előtt létrejött `articles`/`publications`/`layouts`/`deadlines`/`userValidations`/`systemValidations` doc-ok üres `permissions`-szal hagyatkoznak a collection-fallback-ra. Új CF action `backfill_acl_phase3` (vagy a `backfill_acl_phase2` bővítése egy `collections` paraméterrel). **Fallback policy (Codex verifying C5, 2026-05-14)**: per-doc ACL build sorrend (1) ha `doc.createdBy` érvényes user-$id (non-empty string + Auth user létezik) → `withCreator(buildOfficeAclPerms(office.$id), doc.createdBy)`. (2) Ha `createdBy` hiányzik vagy invalid → CSAK office team-perm `buildOfficeAclPerms(office.$id)`, NINCS user-read fallback (NE inferáljunk a `modifiedBy` vagy más mezőből arbitrary usert — ASVS V4.1.3 least privilege + ownership enforcement). `user-read preserve` mint S.7.2 a meglévő `read("user:*")` perm-ekre. **Audit log mandatory**: minden fallback (kategória 2) doc per-collection + per-$id `fallbackUsedDocs: [{collectionId, docId}]` arrayba az action response-ba + CF stdout. **DEPLOY-BLOCKER** S.7.7 close előtt.
...
- [ ] **S.7.9** — GDPR Art. 17 stale `withCreator` user-read (Harden P4, R.S.7.5): user kilépés / törlés UTÁN a `Permission.read(user(callerId))` STALE marad a régi membership/history doc-okon. Fix: új `anonymize_user_acl` CF action — `removeTeamMembership` MELLÉ `updateDocument` permissions a user-id-jét tartalmazó doc-okon (target: minden `createDocument` ahol `withCreator(...)`-rel ment). DryRun + éles mód. **Q1 user-decision (2026-05-13, Harden Phase 7)**: B opció rögzítve — külön action, NEM a `backfill_acl_phase2`-be építve. Indok: 1 felelősség per action, eltérő cadence (backfill 1× a S.7.1 fix után, cleanup havonta/évente). Más debug + hibavektor + retention-policy.
