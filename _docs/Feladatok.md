...
- [/] **S.13.3** — Error message info-disclosure audit. **Phase 1.0 + Phase 1.5 partial close 2026-05-15** (Codex 6 review iter): Phase 1.0 = centralized `fail()` strip + main.js + 2 ad-hoc + 2 defense fix (cycle-safe stripSensitive + reason normalize). Phase 1.5 = `_finalizeOrgIfProvisioning` errorCode rename + `orgs.js:850` cleanupFailed flag + schemas.js `recordError` helper-szintű strip (3 def) + ~13 direct push `, message: err.message` strip + `schemas.js:2966` `backfillMembershipUserNames` fix + `orgs.js:575` callsite migráció. **Phase 2 (külön iter)**: maradék 10-15 CF (`update-article`, `validate-publication-update`, `user-cascade-delete`, stb.). Részletek: [[Komponensek/LoggingMonitoring]].
