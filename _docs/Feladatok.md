...
- [x] **A.2.9** Dashboard frontend: minden workflow-érintő publikáció-mutáció átkötve a CF actionökre — direkt `databases.updateDocument` megkerülné az autoseedet. (Codex stop-time review KRITIKUS):
  ...
  - `CreatePublicationModal.handleSubmit` → `createPublication` workflowId NÉLKÜL + `assign_workflow_to_publication` post-create lépésben. **Rollback ha az assign bukik** (Codex stop-time review KRITIKUS): `deletePublication` + error toast — a publikáció NEM maradhat orphan workflowId nélkül, különben a stack csendben rossz / null workflow-val futna. A layout-create az assign UTÁN fut, hogy a happy-path konzisztens legyen.
