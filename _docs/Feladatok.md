...
- [ ] **A.4.5** `EditorialOfficeGroupsTab` bővítés: csoporthoz `permissionSet` hozzárendelés (multi-select). A régi `DEFAULT_GROUP_SLUGS` UI-flag eltávolítása (a [A.2.7] `delete_group` blocker-set kiváltja a kliens-oldali default-group-védelmet).
...
- [ ] **A.4.9** Dashboard `useContributorGroups` refaktor ([packages/maestro-dashboard/src/hooks/useContributorGroups.js](packages/maestro-dashboard/src/hooks/useContributorGroups.js)): `DEFAULT_ORDER` → aktív publikáció `compiledWorkflowSnapshot.requiredGroupSlugs[]` (vagy a szerkesztett workflow `compiled.requiredGroupSlugs[]`) index-rendezés; metadata (`description`, `color`, `isContributorGroup`, `isLeaderGroup`) a `groups` collection-ből. A `maestro-shared/groups.js` `DEFAULT_GROUPS` deprecated import eltávolítása.
...
- [ ] **A.5.4** Plugin `useContributorGroups` refaktor ([packages/maestro-indesign/src/data/hooks/useContributorGroups.js](packages/maestro-indesign/src/data/hooks/useContributorGroups.js)): mint A.4.9, plugin oldalon — `compiledWorkflowSnapshot.requiredGroupSlugs[]` index-rendezés + metadata a `groups` collection-ből. `DEFAULT_GROUPS` import eltávolítása.
- [ ] **A.5.5** TOCTOU `expectedUpdatedAt` opt-in a Plugin-oldali aktiváló-ban (Fázis 6 után): a plugin a fresh `$updatedAt`-et átadja az `activate_publication` CF-nek a `concurrent_modification` 409 kezeléshez. (Dashboard `GeneralTab` oldali rész már A.2.9-ben kész.)
...
- [ ] **A.7.1** CompiledValidator drift: a [packages/maestro-server/functions/invite-to-organization/src/main.js](packages/maestro-server/functions/invite-to-organization/src/main.js) inline `validateCompiledSlugsInline` helper és a [packages/maestro-shared/compiledValidator.js](packages/maestro-shared/compiledValidator.js) szinkronizálandó. Megoldás: CI-szintű AST-equality teszt vagy single-source build-step refactor (CommonJS bundle a shared modulból).
...
- [ ] **B.0.3** **CF action-bontás (Refactor Fázis 2)** — a B blokk új extension-CRUD action-jeit eleve külön action-fájlokba (`actions/extensions.js`) helyezzük, és ezzel egyidőben a meglévő [`packages/maestro-server/functions/invite-to-organization/src/main.js`](packages/maestro-server/functions/invite-to-organization/src/main.js) (~6946 sor a Fázis 1 helper-extract után, 2026-05-02) 36 action handler-jét szétbontjuk 7-8 modulra:
  ...
  A `main.js` ~300-400 sor lesz: env init + permissionContext + action-router (`actionHandlers[action]({databases, env, payload, ...})` pattern). **Tilos import-irány**: `actions/*` → `helpers/*` → `permissions.js` / `teamHelpers.js`. Visszafelé NEM (Codex flag: ciklikus require csendben fél-inicializált exportokat ad CommonJS-ben). **Indok a B blokkra-időzítésre**: a B blokk amúgy is hoz új CF action-öket, így az új struktúrát fájdalom-mentesen lehet bevezetni — a meglévő deploy-tested action-ök egyúttal portolódnak. Phase 1 (helper-extract, Fázis 1) ezt előkészítette: a 9 inline helper már külön modulokban él (`helpers/{cascade,compiledValidator,workflowDoc,groupSeed,deadlineValidator,constants}.js`). **Becsült végső állapot**: `main.js` ~400 sor, `actions/*` 7-8 modul × 500-1000 sor, `helpers/*` változatlan. Komment-anyag teljesen megőrzött.
