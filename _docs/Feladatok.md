...
- [x] **Verifikáció**: publication-szintű cascade delete smoke teszt sikeres *(2026-04-12)*. Teszt publikáció (36 cikk + 1 layout + 1 deadline) törlése → 43 CF végrehajtás (1 × cascade-delete a pub-ra, 36 × cascade-delete az article-ökre, 6 × article-update-guard mellékhatás), 0 maradék rekord az összes child collection-ben. A `cascade-delete` CF-et újra kellett létrehozni (`functions_delete` → `appwrite push function`), mert az eredeti deployment `live: false` állapotban ragadt.
- [x] **MEGOLDVA — `cascade-delete` CF teljes újraépítés** *(2026-04-12)*: a publication-szintű smoke teszt során kiderült, hogy a CF event plumbing-ja megszakadt. **Három root cause** volt: (1) Az eredeti CF deployment elavult (`live: false`), egyszerű redeploy nem oldotta meg az event subscription-t — teljes törlés + újralétrehozás (`functions_delete` → `appwrite push function`) kellett. (2) Az Appwrite MCP `tables_db_delete_row` tool NEM triggerel document event-eket — csak a közvetlen REST API `DELETE /databases/.../documents/...` hívás. (3) Az összes CF helytelen endpoint-ot használt: `APPWRITE_FUNCTION_ENDPOINT` (belső runtime endpoint) automatikusan a function `role: applications` identitásával hitelesít, felülírva az `APPWRITE_API_KEY`-t — a document CRUD `documents.read` scope hiányát dobta. **Fix**: mind a 10 CF `main.js`-ében `APPWRITE_FUNCTION_ENDPOINT` → `APPWRITE_ENDPOINT` csere (fallback: publikus `cloud.appwrite.io/v1`); `appwrite.json`-ben `cascade-delete` scope-ok bővítése `documents.read/write`-tal; MCP-n keresztül `functions_create_deployment` tarball feltöltéssel. **Smoke teszt sikeres**: article `.delete` event → cascade tüzelt, UserValidations/SystemValidations/Thumbnails cleanup 200 OK, 0 hiba. **Cleanup**: StoryTeszt árvái (24 article + 1 layout + 1 deadline + 38 thumbnail) + `st` pub árvái (4 article + 1 layout + 1 deadline + 4 thumbnail) mind manuálisan takarítva MCP-vel.
...
- [x] **Verifikáció**: plugin-ban nincs pub CRUD, Realtime szinkron működik, dashboard link elérhető — build + dead-code grep tiszta (`createPublication`/`updatePublication`/`deletePublication`/`createLayout`/`createDeadline` híváspontok mindenhol törölve, `tables.updateRow(ARTICLES_COLLECTION_ID, ...)` direkt írás 0 találat, webpack production build 3 warning (size), 0 error) *(2026-04-11)*
- [x] **Follow-up**: cikk-update fail-closed refactor — az új `update-article` CF (HTTP endpoint, `packages/maestro-server/functions/update-article/`) szinkron pre-event validációt végez minden cikk-update mezőre (state, name, filePath, startPage/endPage, pageRanges, contributors, markers, lock, thumbnails). A kliens-oldali `DataContext.updateArticle()` + `WorkflowEngine.executeTransition/toggleMarker/lockDocument/unlockDocument` + `LockManager` orphaned lock cleanup + `useArticles` ghost lock cleanup + `DatabaseIntegrityValidator` autoCorrect mind a közös `callUpdateArticleCF()` helperen keresztül hív (`packages/maestro-indesign/src/core/utils/updateArticleClient.js`). A guard `statePermissions[currentState]` alapján dönt, a `lockType`/`lockOwnerId` mezők fast-path kivétellel futnak (saját lock felvétel/feloldás csoporttagság nélkül is engedélyezett). A legacy `article-update-guard` CF defense-in-depth log-only safety net szerepre csökkentett — továbbra is futtatja a parent publication scope sync-et és a `previousState` data hygiene-t, minden egyéb invariáns-sértést csak log-warningként jelöl. A kliens-oldali legacy path migráció törölve (a dedikált `migrate-legacy-paths` CF átveszi). **Manuális lépés**: az `articles` collection `Update` role-ból a `users` megvonása az Appwrite Console-ban, amikor a `update-article` CF deploy-olva van. *(2026-04-11)*
- [x] **Appwrite Console teendők** (Appwrite MCP-vel végrehajtva *2026-04-11*):
  - [x] `articles` collection `Update` role → `users` permission megvonva (`tables_db_update_table` → `$permissions` most: `create("users")`, `read("users")`, `delete("users")`). Fail-closed `update-article` CF enforcement aktív — a kliensek innentől kizárólag a CF-en keresztül tudnak cikkeket írni.
  - [x] `PUBLICATIONS_COLLECTION_ID` env var — az `invite-to-organization` CF-en (ahol a `delete_organization` és `delete_editorial_office` action-ök futnak) már létezik (létrehozva 2026-04-11 14:44 UTC, `secret: true`). Nem kellett új var-t létrehozni, csak ellenőrizni.
- [x] **Harden pass** *(2026-04-12)*: (1) `update-article` CF lock fast-path biztonsági javítás — `freshDoc.lockOwnerId === null || === userId` ellenőrzés megakadályozza a cross-office lock stealing-et (korábban bármely hitelesített user zárolhatott bármely cikket bármely office-ban); (2) `lockType` enum validáció (`VALID_LOCK_TYPES` Set: `'USER'`, `'SYSTEM'`, `null`); (3) office membership check feltétel nélkülre váltva (lock fast-path is igényli a scope védelmet); (4) kliens-oldali `WorkflowEngine` DRY refactor — 4 azonos catch blokk → `_handleCFError()` helper; (5) `APPWRITE_FUNCTION_ENDPOINT` → `APPWRITE_ENDPOINT` javítás mind a 10 CF `main.js`-ében (a belső endpoint `role: applications` identitást kényszerít, felülírva az API key scope-okat); (6) `cascade-delete` CF `appwrite.json` scope bővítés `documents.read/write`-tal; (7) `update-article` CF env var-ok konfigurálva (7 db, MCP-vel)
- [x] **update-article CF deploy + verifikáció** *(2026-04-12)*: CF `live: true`, `latestDeploymentStatus: ready`. Publication-szintű cascade delete smoke teszt igazolta a teljes lánc működését (ld. Fázis 8 verifikáció).
