...
> **Státusz**: szerver-oldal kész, plugin runtime hátra. Részletes ADR: [[Döntések/0007-workflow-extensions]].
>
> **Részleges implementáció (2026-05-04)**: B.1 (adatmodell) + B.2 (shared kontraktus) + **B.3 (CF CRUD + activate_publication snapshot + post-event guard) ✅ kész** (commit-csomag: f8537d4 → 76430fb a `feature/maestro-redesign` branchen). Hátra: B.4 (plugin `extensionRegistry.js` runtime), B.5 (Designer editor tab).
