...
| R.S.7.6 | `documentSecurity: true` flag NEM verifikált a 6 user-data collection-en (ADR 0014 Layer 1 prerequisite) | HIGH | Med | V4, CIS 3 | Codex stop-time S.7.7 | Frontend/DevOps | TBD | **Open** — S.7.7b al-pont. Lista: `articles`, `publications`, `layouts`, `deadlines`, `userValidations`, `systemValidations`. Ha bármelyik flag NEM `true`, a S.7.7 fix doc-szintű ACL ignorálódik → collection-szintű `read(users)` rétegesen exponál. Manuális Console verify VAGY új CF helper. **DEPLOY-BLOCKER**. |
| R.S.7.7 | S.7.7 ELŐTT létrejött legacy doc-ok üres `permissions`-szal hagyatkoznak a collection-fallback-ra (a S.7.2 backfill NEM fedte le ezt a 6 collection-t) | HIGH | High | V4, V5, CIS 3 | Codex stop-time S.7.7 | Backend | TBD | **Open** — S.7.7c al-pont. Új CF action `backfill_acl_phase3` vagy `backfill_acl_phase2` bővítés a 6 user-data collection-re. `withCreator(buildOfficeAclPerms(office.$id), doc.createdBy ?? owner)` minta + user-read preserve mint S.7.2. **DEPLOY-BLOCKER**. |
| R.S.7.4 | `bootstrap_organization` phantom-org window (~10-100ms): `createDocument(organizations)` SIKER UTÁN + `runRollback` ELŐTT a creator látja a doc-ot a `Permission.read(user(callerId))` azonnali hatása miatt. Ha frontend lekér a doc-ról ebben az ablakban, fantom-org megjelenik a UI-ban | MEDIUM | Low | V4 | Codex Harden adversarial P1 | Backend | TBD | **Open** — S.7.8 al-pont. Fix: `status: 'provisioning'` flag (schema-bővítés, érinti `user-cascade-delete` `status: active\|orphaned` mezőt is) + frontend filter `status === 'active'`-ra. **Cross-cutting**, nagy refactor. |
| R.S.7.5 | `withCreator` stale `Permission.read(user(callerId))` user kilépés / törlés UTÁN → GDPR Art. 17 sérülés (a user továbbra is lát a saját history-jét) | MEDIUM | Med | V4, GDPR | Codex Harden adversarial P4 | Backend | TBD | **Open** — S.7.9 al-pont. Fix: új `anonymize_user_acl` CF action — `removeTeamMembership` MELLÉ `updateDocument` permissions a user-id-jét tartalmazó doc-okon. **Q1 user-decision (2026-05-13, Harden Phase 7)**: B — külön action, NEM `backfill_acl_phase2`-be építve. Lásd [[Döntések/0014-tenant-doc-acl-with-creator]]. |
