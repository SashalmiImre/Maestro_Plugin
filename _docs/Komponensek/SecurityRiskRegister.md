...
| R.S.7.2 | `document_already_exists` race-fallback ágon a legacy doc permissions értéke ismeretlen (üres-permission doc backfill hiányzik) | MEDIUM | Low | V4 | Codex stop-time review | Claude | TBD | **Open** — `backfill_acl_phase2` CF action vázlatban (`updateDocument({permissions: ...})` a 8 collection-en, dryRun → éles). Nincs éles adat, fejlesztői env-en eseti reset elegendő. |
| R.S.7.3 | `articles.createDocument` frontend-en (plugin/dashboard direkt Appwrite SDK), NEM a CF-eken keresztül — a `permissions` paraméter NEM állítva → cross-tenant article-push szivárgás | HIGH | High | V4, V5, CIS 3 | Codex Explore | Frontend | TBD | **Open** — S.7.7 al-pont, plugin + dashboard frontend `articles.createDocument` hívásokon `withCreator(buildOfficeAclPerms(officeId), userId)` pótlása. `validate-article-creation` post-event-validator NEM teljes védelem (race-ablak). |
...
| R.S.7.5 | `withCreator` stale `Permission.read(user(callerId))` user kilépés / törlés UTÁN → GDPR Art. 17 sérülés (a user továbbra is lát a saját history-jét) | MEDIUM | Med | V4, GDPR | Codex Harden adversarial P4 | Backend | TBD | **Open** — S.7.9 al-pont. Fix: új `anonymize_user_acl` CF action — `removeTeamMembership` MELLÉ `updateDocument` permissions a user-id-jét tartalmazó doc-okon. Lásd [[Döntések/0014-tenant-doc-acl-with-creator]]. |
