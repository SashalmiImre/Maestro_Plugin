...
module.exports = async function ({ req, res, log, error }) {
    try {
        ...
        //
        // **Auth (átmeneti)**: most org owner/admin (mint a `create_group`).
        // A.3.6 retrofitben ez `userHasPermission('permissionSet.create',
        // editorialOfficeId)`-re fog változni — addig az org-role override
        // ad teljes CRUD-jogot.
        //
        // **Validáció** (A.3.3 + Codex review):
        //   - `name`, `slug`, `permissions[]` kötelező
        //   - `permissions[]`: csak office-scope slug — `org.*` 400
        //     `org_scope_slug_not_allowed`. Ismeretlen slug → 400 `unknown_slug`.
        //     Duplikátum → 400 `duplicate_slug` (a kliens-oldali normalizálás
        //     ezt megfogná, de defense-in-depth).
        //   - `slug` egyedi az office-on belül (`office_slug_unique` index).
        //   - `description` opcionális (max 500 char), `null` = törlés.
        ...
        // a `groupPermissionSets` junction docok és a kliens-cache a slug-ot
        // azonosítóként használja.
        //
        // Frissíthető mezők (mind opcionális, undefined = no-op):
        //   - `name` (max 100 char)
        //   - `description` (max 500 char, null = törlés)
        //   - `permissions[]` (validálva mint create-nél)
        if (action === 'update_permission_set') {
            ...
            let setDoc;
            try {
                setDoc = await databases.getDocument(databaseId, permissionSetsCollectionId, permissionSetId);
            } catch (err) {
        ...
        // ════════════════════════════════════════════════════════
        //
        // Soft-delete a permission set-en (`archivedAt` set/null). Idempotens
        // (`already_archived` / `already_active`).
        //
        // **Codex review (b) opció**: NINCS blocker-set — a `groupPermissionSets`
        // junction docok intaktak maradnak. Az archivált permission set-eket
        // a `userHasPermission()` snapshot-build a `archivedAt === null` szűrővel
        // hagyja figyelmen kívül; restore esetén automatikusan visszaáll a hatás.
        // Ez a megközelítés:
        //   - reverzibilis (mint az archive_workflow)
        //   - engedmény-logikájú (a permission set hiánya nem ad jogot, csak
        //     elveszi)
        //   - egyszerű (nincs cascade törlés a junction-okon)
        //
        // **Auth (átmeneti)**: org owner/admin (A.3.6: `permissionSet.archive`).
        ...
        // index megvédi a duplikátumtól — idempotens (`already_assigned`).
        //
        // **Validáció**:
        //   - groupId és permissionSetId ugyanahhoz az `editorialOfficeId`-hez
        //     tartozik (cross-office assign tiltott).
        //   - Permission set NEM lehet archivált (warning, de nem blokk: a
        //     restore-ra előretolva érvényessé válik. Codex (b) opció).
        //
        // **Auth (átmeneti)**: org owner/admin (A.3.6: `permissionSet.assign`).
        if (action === 'assign_permission_set_to_group') {
            ...
            if (!permissionSetsCollectionId || !groupPermissionSetsCollectionId) {
                return fail(res, 500, 'misconfigured', {
                    missing: [
                        !permissionSetsCollectionId ? 'PERMISSION_SETS_COLLECTION_ID' : null,
                        !groupPermissionSetsCollectionId ? 'GROUP_PERMISSION_SETS_COLLECTION_ID' : null
                    ].filter(Boolean)
                });
            }
            ...
            try {
                [groupDoc, setDoc] = await Promise.all([
                    databases.getDocument(databaseId, groupsCollectionId, groupId),
                    databases.getDocument(databaseId, permissionSetsCollectionId, permissionSetId)
                ]);
            } catch (err) {
                if (err?.code === 404) {
