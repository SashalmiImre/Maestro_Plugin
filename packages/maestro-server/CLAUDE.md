...
**Idempotens workflowExtensions schema bootstrap** — `bootstrap_workflow_extension_schema` CF action (B.1.1 / ADR 0007 Phase 0):
- Owner-only (any org-ban owner role elég).
- Új collection `workflowExtensions` idempotens létrehozása `documentSecurity: true` flaggel — a doc-szintű team ACL ad olvasási jogot (`buildExtensionAclPerms`, `teamHelpers.js`). A 409 / "already exists" → `skipped`, a hiba propagál minden más esetre.
- `workflowExtensions` attribútumok: `name` (100), `slug` (64), `kind` (enum: `validator | command`), `scope` (enum: **csak `article` Phase 0-ban**, default `article`), `code` (string ~1 MB), `visibility` (enum 3-way mint workflow), `archivedAt` (datetime nullable), `editorialOfficeId` (36), `organizationId` (36), `createdByUserId` (36, nullable). **Phase 0 hatókör-szűkítés (B.0.4)**: a `paramSchema` mező SZÁNDÉKOSAN nincs benne; a `scope` enum is fail-closed `['article']`-re szűkítve — a Phase 1+ `publication` scope egy `updateEnumAttribute`-tal kerül be a `bootstrap_workflow_schema` `public` visibility late-add mintáját követve, ezzel a B.3 CRUD action-nek nem kell explicit `scope: 'publication'` guardot adnia.
- Indexek: `office_slug_unique` (`(editorialOfficeId, slug)` unique — autoseed/duplikátum-blokk), `office_idx`, `org_idx`. A Phase 0 office-szintű uniqueness elegendő: az `ext.<slug>` resolver office-szűkített (a Plugin a workflow office-ban keresi az extension-eket).
- Action-szintű env var guard: `WORKFLOW_EXTENSIONS_COLLECTION_ID` — Phase 0-ban CSAK ezen az action-en kötelező; a B.3 új CRUD action-jeinek (`create/update/archive_workflow_extension`) érkezésekor globális fail-fast-ba emelendő.
- Aszinkron Appwrite attribute processing miatt az index-create első futáson 400-zal elbukhat (`indexesPending`) — a user 10s múlva újra futtatja az action-t.
- Deploy után a Console-on ellenőrizendő: a `workflowExtensions` collection `rowSecurity` flag-je aktív (különben a doc-szintű ACL nem érvényesül a Realtime push-on, ld. ADR 0003 deploy checklist).
...
- `delete_workflow` — `{ editorialOfficeId, workflowId }`, org owner/admin auth. **Blocking scan**: a workflow-ra hivatkozó publikációk listája (`publications.workflowId === workflowId`) — ha van, `workflow_in_use` + `usedByPublications: [{ $id, name, editorialOfficeId, organizationId }, ...]`. **Hatókör 3-ágú visibility switch (2026-05-04 fix, B.0.3.f review)**: `visibility='public'` → cross-org scan (nincs scope-szűrő, csak `workflowId` egyezés — a `public` workflow-kat a `create_publication_with_workflow` és `assign_workflow_to_publication` cross-org engedi, a delete-blockernek ezt fednie kell); `visibility='organization'` → az egész org minden office-án belül; `visibility='editorial_office'` → csak a saját office. **Invalid / null visibility → fail-closed `public` scope** (LEGSZÉLESEBB scan, Codex review P2 2026-05-04): a delete-blocker célja minden hivatkozó publikációt megtalálni; ha a visibility legacy / korrupt / manuális Console-edit miatt ismeretlen, az office-szűkítés data-loss kockázatot teremt, ezért a teljes scan a fail-closed default. Cap: `MAX_REFERENCES_PER_SCAN=50` (korai kilépés), `CASCADE_BATCH_LIMIT=100` (paginált listDocuments). A hard-delete a `cleanup-archived-workflows` scheduled CF-en keresztül automatizált — a 7 napon túl archivált workflow-kra.
