...
| **Layout Változott**        | `layoutChanged`              | `ArticleProperties`, `DataContext` (layouts Realtime) | `{ article }`, `{ articles, publicationId }` vagy `{ publicationId }` | Layout változáskor (egyedi vagy Dashboard-oldali Realtime esemény) tüzel. Per-publikáció 250 ms debounce-szal koalesszkálja a bulk műveleteket. Átfedés validációt indít. |
| **Lefedettség Változott**   | `publicationCoverageChanged` | `DataContext` (publications Realtime)              | `{ publication }`                  | A kiadvány `coverageStart`/`coverageEnd` mezőjének változásakor tüzel (Dashboard szerkesztés → Realtime). A teljes frissített publikáció payload-ot tartalmazza, hogy a handler a friss `coverage*` mezőkkel validáljon. A kiadvány összes cikkére újraszámolja az átfedés validációt. |
| **Cikkek Hozzáadva**        | `articlesAdded`              | `Publication`                                      | `{ publicationId, articles }`    | Új cikkek hozzáadása után tüzel. A `articles` tömb a létrehozott cikk objektumokat tartalmazza (közvetlenül a DB válaszból), hogy a handler ne függjön a React state frissüléstől. Átfedés/struktúra validációt indít (PublicationStructureValidator). |
| **Állapot Változott**       | `stateChanged`               | `WorkflowEngine`                                   | `{ article, previousState, newState }` | Sikeres workflow átmenet után tüzel. A `article` payload a CF szinkron válaszából származik (legfrissebb állapot); a `useWorkflowValidation` handler `articlesRef.current.some($id)` existence check-kel szűri a közben eltűnt (pub switch / törlés) cikkeket. |
