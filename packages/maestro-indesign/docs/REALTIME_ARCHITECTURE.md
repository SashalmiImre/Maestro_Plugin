...
*   **Kapcsolat-deduplikáció & csatorna-nyilvántartás**: A `createSocket` az `activeSubscriptions` Map-ből kinyeri az összes csatornát. Ha van már `CONNECTING` vagy `OPEN` állapotú socket és nem érkeztek új csatornák, nem épül újra. Ha új csatornák érkeznek (pl. az `account` csatorna a database channels után), a régi socketet lezárja és újat hoz létre az összes csatornával. A `_subscribedChannels` Set nyomon követi az aktuális socket csatornáit.
*   **v24 per-slot query paraméterek**: A `createSocket` minden feliratkozás (slot) számára per-slot query paramokat generál (`channel[slot][]=query`), amelyek alapján a szerver subscription ID-kat rendel a slotokhoz. A `handleResponseConnected()` eltárolja ezeket, a `handleResponseEvent()` pedig az event routing-hoz használja.
